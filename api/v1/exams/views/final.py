...
from api.v1.exams.serializers.final import FinalExamAnswerSerializer
from api.v1.exams.views.categories import CategoryMixExamAnswerAPIView
...
class FinalExamAPIView(GenericAPIView):
    ...
    def get_queryset(self):
        return Question.objects.filter(questiondetail__language_id=get_language()).order_by('?'
                                                                                            )[:settings.FINAL_QUESTIONS]
...
class FinalExamAnswerAPIView(CategoryMixExamAnswerAPIView):
    serializer_class = FinalExamAnswerSerializer
